...
fun initTime(env: Env) {
    env.symbol.table.putAll(
        mutableMapOf(
            ...
            "time-format" to (HimeFunction(env, FuncType.BUILT_IN, fun(args: List<Token>, _: SymbolTable): Token {
                himeAssertType(args[1], "int", env)
                return SimpleDateFormat(args[0].toString()).format(args[1].toString().toLong())
                    .toToken(env)
            }, 2)).toToken(env),
            "time-parse" to (HimeFunction(env, FuncType.BUILT_IN, fun(args: List<Token>, _: SymbolTable): Token {
